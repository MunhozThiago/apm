 ...
 */
function ReplaceMenuProvider(popupMenu, modeling, moddle, bpmnReplace, rules, bpmnFactory) {
...
ReplaceMenuProvider.$inject = [ 'popupMenu', 'modeling', 'moddle', 'bpmnReplace', 'rules', 'bpmnFactory' ];
...
ReplaceMenuProvider.prototype.getEntries = function(element) {
  ...
  if (is(businessObject, 'bpmn:Participant')) {
    return this._createParticipantEntries(element, replaceOptions.PARTICIPANT);
  }
...
ReplaceMenuProvider.prototype._createParticipantEntries = function (element, replaceOptions) {

  var businessObject = getBusinessObject(element);
  var menuEntries = [];
  var replaceElement = this._bpmnReplace.replaceElement;
  var create = this._bpmnFactory.create;
  var self = this;

  function replaceParticipant() {
    replaceElement(element, { type: 'bpmn:Participant'});
  }

  forEach(replaceOptions, function(entry) {
      switch(entry.actionName) {
        case 'replace-with-expanded-pool':
          if (!isExpanded(businessObject)) {
            menuEntries.push(self._createMenuEntry(entry, element, replaceParticipant));
          }
          break;
        case 'replace-with-collapsed-pool':
          if (isExpanded(businessObject)) {
            menuEntries.push(self._createMenuEntry(entry, element, replaceParticipant));
          }
          break;
