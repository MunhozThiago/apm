...
function ReplaceElementBehaviour(eventBus, bpmnReplace, bpmnRules, elementRegistry, selection, modeling) {
  ...
  this.postExecuted([ 'shape.replace' ], 100, function(e) {
    var context = e.context,
        oldShape = context.oldShape,
        newShape = context.newShape;

    console.log("pre replace, new shape", newShape);
    console.log("pre replace, old", oldShape);

    if(is(newShape, 'bpmn:Participant') && !isExpanded(newShape)) {
      // newShape.children = [];
      //modeling.removeElements( newShape.children );
      modeling.removeElements( newShape.children );

      console.log(newShape.children);
      forEach(newShape.children, function(child){
      });
    }
