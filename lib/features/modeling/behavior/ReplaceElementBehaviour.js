...
function ReplaceElementBehaviour(eventBus, bpmnReplace, bpmnRules, elementRegistry, selection, modeling) {
  ...
  // replace collapsed and expanded shape
  this.preExecute( [ 'shape.replace'], 100, function(e){
      var context = e.context,
          oldShape = context.oldShape,
          newShape = context.newShape;

      if(is(oldShape, 'bpmn:Participant') && isExpanded(oldShape)) {

        // first remove lanes from old expanded participant
        modeling.removeElements(getChildLanes(oldShape));

        // then remove the other children from old shape
        saveClear(oldShape.children, function(e) {
